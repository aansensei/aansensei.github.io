...
<style>
  ...
  /* ======================================================= */
  /* HIỆU ỨNG SÁCH 3D (ĐÃ SỬA CÁCH ĐỊNH VỊ)                 */
  /* ======================================================= */
  ...
  .book-3d-wrapper {
    ...
    position: absolute; 
    /* LUÔN GẮN SÁCH Ở GIỮA MÀN HÌNH */
    top: 50%; left: 50%; 
    transform-style: preserve-3d;
    /* Transform sẽ được JS lo liệu toàn bộ */
  }
...
<script>
  ...
  function triggerBookOpen(url, bgGradient, title, event) {
    ...
    // TOÁN HỌC: Lấy tọa độ chuột và tính độ lệch so với Tâm màn hình
    const mouseX = event.clientX;
    const mouseY = event.clientY;
    const centerX = window.innerWidth / 2;
    const centerY = window.innerHeight / 2;
    
    // Khoảng cách cần dịch chuyển để sách nằm đúng đầu mũi chuột
    const moveX = mouseX - centerX;
    const moveY = mouseY - centerY;

    // TẮT CHUYỂN ĐỘNG: Đưa quyển sách thu nhỏ (0.01) đến đúng mũi chuột ngay lập tức
    book.style.transition = 'none';
    ...
    // XÓA BỘ NHỚ ĐỆM CSS (Buộc trình duyệt đặt sách ở chuột trước khi bay)
    void book.offsetWidth;

    // BẬT LẠI CHUYỂN ĐỘNG: Bay về giữa (-50%, -50%) và phóng to ra
    setTimeout(() => {
